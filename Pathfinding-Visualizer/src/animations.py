...
class Animator:
    ...
    def animate_nodes(self):
        ...
        for center in list(self.nodes_to_animate.keys()):
            ...
            if (row, col) == self.maze.start:
                ...
                text = FONT_14.render(
                    str(cost), True, Light_Slate_Gray
                )
    ...
    def _weight_animation(self, node: AnimatingNode) -> None:
        ...
        # Draw
        pygame.draw.rect(self.surface, Light_Slate_Gray, node.rect)
        pygame.draw.rect(self.surface, Teal, node.rect, width=8)
    ...
    def _path_animation(self, node: AnimatingNode) -> None:
        ...
        if node.progress > 0.50 * node.duration:
            pygame.draw.rect(
                self.surface,
                Sea_Green if node.progress < 0.75 * node.duration else Light_Slate_Gray,
                node.rect,
                border_radius=border_radius,
                width=1
            )
