...
class Maze:
    ...
    def set_cell(self, pos: tuple[int, int], value: str, forced: bool = False) -> None:
        ...
        if pos in (self.start, self.goal) and not forced:
            if value == "V":
                self.maze[pos[0]][pos[1]].color = Sky_Blue
            elif value == "*":
                self.maze[pos[0]][pos[1]].color = Gold
            return
        ...
        for cell in solution.explored:
            ...
            nodes.append(
                AnimatingNode(
                    ...
                    value="V",
                    color=Ivory,
                    colors=[Gold, Royal_Blue, Cornflower_Blue, Mint_Green, Sky_Blue],
                    duration=1500,
    ...
    def _draw_rect(
            ...
            color: tuple[int, int, int] = Sky_Blue,
        ...
        Args:
            ...
            color (tuple[int, int, int], optional): Color. Defaults to Gold.
        ...
        if coords in (self.start, self.goal) and color == Teal:
            return
        ...
        if color in (Sky_Blue, Ivory, Sea_Green) or \
                (node and node.color == Gold):
            pygame.draw.rect(
                surface=self.surface,
                color=Sea_Green if color == Mint_Green else Light_Slate_Gray,
        ...
        if (n := self.maze[row][col]).cost > 1:
            ...
            text = FONT_14.render(
                str(n.cost if not node else node.value), True, Light_Slate_Gray
            )
