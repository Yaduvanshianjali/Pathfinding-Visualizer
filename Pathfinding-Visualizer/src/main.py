...
title = Label(
    ...
    background_color=pygame.Color(*Midnight_Blue),
    foreground_color=pygame.Color(*Ivory),
...
visualise_btn = Button(
    "VISUALISE", "center", 0,
    background_color=pygame.Color(*Sea_Green),
    foreground_color=pygame.Color(*Ivory),
...
def main() -> None:
    ...
    state.label = Label(
        ...
        background_color=pygame.Color(*Ivory),
        foreground_color=pygame.Color(*Teal),
        padding=6, font_size=20, outline=False,
        surface=WINDOW,
    )
    ...
    state.speed_label = Label(
        ...
        foreground_color=pygame.Color(*Ivory),
        background_color=pygame.Color(*Cornflower_Blue),
    )
...
def draw() -> None:
    ...
    WINDOW.fill(Ivory)
    pygame.draw.rect(WINDOW, Midnight_Blue, top)
    title.draw()

    # Draw maze legend
    texts = {
        "Start Node": Ivory,
        "Visited Node": Sky_Blue,
        "Shortest-Path Node": Gold,
        "Unvisited Node": Ivory,
        "Wall Node": Teal,
        "Weighted Node": Ivory,
        "Target Node": Ivory,
    }
    ...
    for text in texts:
        ...
        pygame.draw.rect(WINDOW, Light_Slate_Gray, (x, y, 30, 30), width=1)

        # Text (Meaning)
        text_surf = FONT_18.render(text, True, Teal)
...
def show_results(results: list[tuple[str, dict[str, float]]]) -> None:
    ...
    colors = [Mint_Green, Mint_Green, Gold, Gold]
    colors.extend([Light_Slate_Gray] * (len(results) - 4))
    ...
    popup = Popup(
        ...
        children=[
            Label(
                ...
                background_color=pygame.Color(*Teal),
                foreground_color=pygame.Color(*Ivory),
